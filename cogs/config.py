...
class Config(commands.Cog):
    ...
    async def welcome(self, ctx: commands.Context):
        ...
        if config.welcome_enabled:
            welcome_channel = discord.utils.get(ctx.guild.channels, id=welcome_config.channel_id)
            return await ctx.send(
    ...
    async def setwelcome(self, ctx: commands.Context):
        ...
        await ctx.send("Please enter the channel where all the welcome messages should be sent.")
        ...
        if not welcome_config:
            new_welcome_config = WelcomeConfig(guild_id=ctx.guild.id, channel_id=channel.id, message=welcome_msg)
            ...
            return await ctx.send(f"Updated welcome config. All member join events will be sent to {channel.mention}.")
    ...
    async def leave(self, ctx: commands.Context):
        ...
        if config.leave_enabled:
            leave_channel = discord.utils.get(ctx.guild.channels, id=leave_config.channel_id)
            return await ctx.send(
    ...
    async def setleave(self, ctx: commands.Context):
        ...
        await ctx.send("Please enter the channel where all the leave messages should be sent.")
        ...
        if not leave_config:
            new_leave_config = LeaveConfig(guild_id=ctx.guild.id, channel_id=channel.id, message=leave_msg)
            ...
            return await ctx.send(f"Updated leave config. All member leave events will be sent to {channel.mention}.")
